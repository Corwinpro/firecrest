...
def bspline_boundary_element():
    boundary = BSplineElement(control_points, bcond, el_size, degree=degree)
    return boundary
...
def test_boundary_surface_index(bspline_boundary_element):
    ...
    next_boundary = BSplineElement(control_points, bcond, el_size)
